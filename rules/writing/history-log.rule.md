...
- A project is a folder holding HISTORY.jsonl, TASKS.md or QUESTIONS.jsonl; start one with rules.py init <folder> --goal G, the repo root included (init .). Projects nest: records go to the nearest project file up the path; a sub-project names its parent task: init <sub> --task <id>. Work in a subfolder goes into its project's file, tagged with the subfolder.
