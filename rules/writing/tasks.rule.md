...
- The window: ✅ done, at most 2, date at the end. 🔄 in progress, exactly 1, bold, "since <date>". ⛔ blocked, reason in its details. 🔜 not started, any number, no date. ❓ after the status icon marks an open question.
...
- While a viewer is open (rules.py view --status), questions for the user go to it, not to chat; run tasks wait in the background to be told. No viewer open: ask in chat.
