...
**t29** · graphify (graphifyy 0.9.63), in use in this repo. Install script runs graphify install, mines and deletes its two instruction files, moves its hooks into the code specialist definition, passes --out into the specialist's home. Live use: its nudge names bare graphify, so a shim or PATH entry is required, and its Grep nudge says MANDATORY even for exact lookups. Depends on t63. Done when a specialist queries a graph and the rule fires.
...
**t71** · caveman: output style for the manager, compact rules in each agent body. rtk 0.49.0: pinned binary, mined rule, filters kept, Bash hook in workspace settings, telemetry off. Live use here: the hook emits bare rtk, so the launcher must put workspace tools on PATH; python -m pytest becomes rtk pytest, escaping python allow rules; Bash only. Needed: install script, PATH, test-runner exclusions, rtk data folders. Done when output is condensed and failures stay readable.
