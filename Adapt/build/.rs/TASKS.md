...
**t14** · Create the manager's records in manager_home with the vendored rules-system: rules.py init <manager_home> --no-questions, which writes only the history and task files into manager_home/.rs/. Done when rules.py tasks on manager_home shows an empty window with the manager's goal and no questions file exists.
