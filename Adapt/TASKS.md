...
**t9** · Blocked: a user action: only the user can accept the trust dialog · Claude Code ignores a project's allow list until the workspace is trusted; hooks run regardless. The user opens Claude Code in the Adapt folder once and accepts the trust dialog. Done means a headless session there no longer prints the 'workspace has not been trusted' warning.
